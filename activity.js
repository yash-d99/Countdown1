...
const bookList = [
  ...
  const addition = (book) => bookList.push(book)

  const removal = (book) => { bookList.splice(bookList.indexOf(book),1)}

  const listout = () => bookList.forEach((book) => console.log(book))
...
const sign = (number) => console.log(number>0=== true ? 'positive' : 'negative')
